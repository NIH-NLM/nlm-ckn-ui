...
All application secrets (ArangoDB password, Django secret key, CloudFront origin secret) are generated by a Lambda-backed CloudFormation custom resource on first deploy. Secrets are stable across stack updates — re-deploying will never rotate them. They are passed between stacks as `NoEcho` parameters and never appear in CloudFormation outputs.
