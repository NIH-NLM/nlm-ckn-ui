...
test('Graph page shows two selected nodes and builds graph with both origins', async ({ page }) => {
    ...
    await page.route('**/arango_api/sunburst/', async (route) => {
        if (route.request().method() === 'POST') {
            // Single root object
            return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(mockTree) });
        }
        return route.continue();
    });
    ...
    // Click add button
    await page.locator('.unified-search-results-list .result-item-row-link .item-meta-actions .add-to-graph-button').click();
