...
test('About page loads and displays content', async ({ page }) => {
    ...
    // Check title
    await expect(page.locator('h1.content-page-title')).toHaveText('About the NLM Cell Knowledge Network');

    // Check content text
    await expect(page.locator('.about-text-section')).toContainText('National Library of Medicine (NLM) Cell Knowledge Network');
